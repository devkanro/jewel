...
fun Tables() {
    ...
    val draggableColumnState =
        rememberLazyTableColumnDraggingState(
            ...
            onMove = { from, to ->
                val fromIndex =
                    columns.indexOf(from).takeIf { it >= 0 } ?: return@rememberLazyTableColumnDraggingState false
                val toIndex =
                    columns.indexOf(to).takeIf { it >= 0 } ?: return@rememberLazyTableColumnDraggingState false
    ...
    val draggableRowState =
        rememberLazyTableRowDraggingState(
            ...
            onMove = { from, to ->
                val fromIndex = rows.indexOf(from).takeIf { it >= 0 } ?: return@rememberLazyTableRowDraggingState false
                val toIndex = rows.indexOf(to).takeIf { it >= 0 } ?: return@rememberLazyTableRowDraggingState false
