    ...
    awaitEachGesture {
        ...
        if (awaitTouchSlop) {
            do {
                drag = awaitPointerSlopOrCancellation(
                    down.id, down.type, orientation = orientationLock
                ) { change, over ->
                    change.consume()
                    overSlop = over
                }
        ...
        if (drag != null) {
            ...
            val upEvent = drag(pointerId = drag.id, onDrag = {
                ...
                it.consume()
            }, orientation = orientationLock, motionConsumed = {
                it.isConsumed
            })
            if (upEvent == null) {
                onDragCancel()
...
internal class TouchSlopDetector(val orientation: Orientation? = null) {
    ...
    private fun calculatePostSlopOffset(touchSlop: Float): Offset {
        return if (orientation == null) {
            val touchSlopOffset = totalPositionChange / totalPositionChange.getDistance() * touchSlop
